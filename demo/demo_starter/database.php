...
class TxDatabase {
     ...
     * @return XiuDatabase
     */
    public function getConn($dbname = "xiu_user", $isSlave = false) {
        if ($isSlave) {
            $dbname .= '_slave';
        }
     ...
     */
    protected function _connect($dbname = "xiu_user") {
        try {
            //获取数据库配置
            require( SYSLIB . "config/database.php");
            $dbconfig = $db[$dbname];
